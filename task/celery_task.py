...
from pelatihanindoprima.crews.content_crew.content_crew import ContentCrew
from pelatihanindoprima.crews.analisator.analisator import Analisator
from pelatihanindoprima.crews.recomender.recomender import Recommender
from pelatihanindoprima.crews.file_analyzer.file_analyzer import FileAnalyzer
...
@celery_app.task(bind=True, name="forecast-excel")
def predict_excel(self, file: str, periods: int = 30):

    self.update_state(
        state='RUNNING',
        meta={'current': f'Forecast processing for {file}'}
    )
